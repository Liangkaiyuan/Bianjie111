...
public class IndexServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			sql="select * from t_news where isHot=1 order by publishDate desc limit 0,6 ";
			...
			if(newsTypeList!=null && newsTypeList.size()!=0){
				for(int i=0;i<newsTypeList.size();i++){
					...
					sql="select * from t_news,t_newsType where typeId=newsTypeId and typeId="+newsType.getNewsTypeId()+" order by publishDate desc limit 0,6";
